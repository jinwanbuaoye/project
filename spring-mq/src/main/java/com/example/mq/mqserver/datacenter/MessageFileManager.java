...
public void sendMessage(MSGQueue queue, Message message) throws MqException, IOException {
    ...
    public void deleteMessage(MSGQueue queue, Message message) throws IOException, ClassNotFoundException {
        synchronized (queue) {
            /**
             * 之前用过的FileInputStream和FileOutputStream都是从文件头读写的，
             * 而此处我们需要的是，在文件中的指定位置，进行读写.随机访问，
             * 此处用到的类，RandomAccessFile
             * 使用 RandomAccessFile 类进行基于偏移量（offset）的随机文件读取操作
             */
            try (RandomAccessFile randomAccessFile = new RandomAccessFile(getQueueDataPath(queue.getName()), "rw")) {
                // 1. 先从文件中读取对应的 Message 数据.
                byte[] bufferSrc = new byte[(int) (message.getOffsetEnd() - message.getOffsetBeg())];
                randomAccessFile.seek(message.getOffsetBeg());//将文件指针移动到指定的起始位置
                randomAccessFile.read(bufferSrc);//从当前文件指针的位置开始，读取数据到字节数组 bufferSrc 中
